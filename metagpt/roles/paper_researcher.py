...
from metagpt.actions.paper_research import GeneratePaperSummary, RankPapers, SearchAcademicPapers
...
class PaperResearcher(Role):
    ...
    constraints: str = Field(default="Ensure information sources are reliable and citations are accurate")
    language: str = Field(default="en", description="Output language: 'en' or 'zh'")
    research_config: ResearchConfig = Field(default_factory=ResearchConfig, description="Research configuration")
    ...
    async def _run_search(self, topic: str) -> str:
        ...
        self._search_action = SearchAcademicPapers(
            max_results=config.max_papers, min_citations=config.min_citations, year_range=config.year_range
        )
        self._search_action.set_context(self.context)
        self._search_action.set_llm(self.llm)

        result = await self._search_action.run(
            topic=topic, sources=["semantic_scholar", "arxiv"], language=config.language
        )
        ...
        for pid in paper_ids[:5]:  # 限制摘要数量以节省成本
            try:
                ...
                summary = await summary_action.run(
                    paper_id=pid, source="arxiv", language=self.research_config.language, paper_data=paper_data
                )
    ...
    async def _rank_papers(self, topic: str, papers_info: list[dict]) -> str:
        ...
        # 排序
        rank_action = RankPapers(default_dimension=self.research_config.ranking_dimension)
        rank_action.set_context(self.context)
        rank_action.set_llm(self.llm)

        ranked_report = await rank_action.run(
            papers=papers, topic=topic, language=self.research_config.language, top_k=self.research_config.top_k
        )
    ...
    def _generate_recommendations(self, papers_info: list[dict]) -> list[str]:
        ...
        if recent:
            suggestions.append(f"Recent papers ({len(recent)} papers from recent years)")

        # 开源论文
        open_access = [p for p in papers_info if p.get("pdf_url")]
        if open_access:
            suggestions.append(f"{len(open_access)} papers have free PDF access")
