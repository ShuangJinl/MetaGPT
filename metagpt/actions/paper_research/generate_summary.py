...
class GeneratePaperSummary(Action):
    ...
    async def run(
        self, paper_id: str, source: str = "arxiv", language: str = "en", paper_data: Optional[Paper] = None
    ) -> str:
        """
        生成单篇论文摘要
    ...
    async def _generate_standard_summary(self, paper: Paper, language: str) -> str:
        """生成标准摘要"""
        # 构建提示
    ...
    async def _generate_extended_abstract(self, paper: Paper, language: str) -> str:
        """生成扩展摘要"""
    ...
    async def run_batch(self, paper_ids: list[str], source: str = "semantic_scholar", language: str = "en") -> str:
        """
        批量生成论文摘要
        ...
        # 并行生成摘要
        tasks = [self.run(paper_id, source, language) for paper_id in paper_ids]
    ...
    async def run_comparative(
        self, paper_ids: list[str], source: str = "semantic_scholar", language: str = "en"
    ) -> str:
        """
        生成对比摘要
        ...
        try:
            ...
            # 生成对比分析
            prompt = COMPARATIVE_SUMMARY_PROMPT.format(papers_info="\n\n".join(papers_info), language=language)

            comparison = await self._aask(prompt)

            # 构建完整报告
            report = ["# Comparative Paper Summary", "", f"Analyzing {len(papers)} papers", "", "---", "", comparison]
    ...
    async def generate_key_points(self, paper_id: str, source: str = "semantic_scholar", language: str = "en") -> str:
        """
        生成论文要点列表（简洁版本）
