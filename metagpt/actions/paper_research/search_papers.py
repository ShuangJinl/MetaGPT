...
class SearchAcademicPapers(Action):
    ...
    year_range: Optional[tuple[int, int]] = Field(
        default=None, description="Publication year range (min_year, max_year)"
    )
    ...
    async def run(self, topic: str, sources: Optional[list[str]] = None, language: str = "en") -> str:
        """
        执行论文搜索
        ...
        try:
            ...
            result = await self.academic_search.search_multiple(
                query=search_topic, max_results=self.max_results, sources=sources
            )
    ...
    async def search_by_author(self, author_name: str, max_results: int = 10) -> str:
        """
        根据作者搜索论文
        ...
        result = await self.academic_search.search_semantic_scholar(query=query, max_results=max_results)
    ...
    async def search_by_doi(self, doi: str) -> str:
        ...
        result = await self.academic_search.search_semantic_scholar(query=f"DOI:{doi}", max_results=1)
