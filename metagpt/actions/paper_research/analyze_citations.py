...
class AnalyzeCitations(Action):
    ...
    async def run(self, paper_id: str, source: str = "semantic_scholar", language: str = "en") -> str:
        """
        分析单篇论文的引用关系
        ...
        try:
            ...
            # 生成分析报告
            return await self._generate_analysis_report(paper, references, cited_by, metrics, language)
    ...
    def _calculate_metrics(self, paper: Paper, references: list[Paper], cited_by: list[Paper]) -> CitationMetrics:
        """计算引用指标"""
        # 计算引用速度（每年引用数）
        ...
        # 计算影响力比率
        influence_ratio = paper.influential_citation_count / paper.citation_count if paper.citation_count > 0 else 0
    ...
    async def _generate_analysis_report(
        self, paper: Paper, references: list[Paper], cited_by: list[Paper], metrics: CitationMetrics, language: str
    ) -> str:
        """生成分析报告"""
        # 格式化参考文献
    ...
    async def run_network_analysis(self, paper_ids: list[str], language: str = "en") -> str:
        """
        对多篇论文进行引用网络分析
        ...
        try:
            ...
            # 生成网络分析报告
            prompt = NETWORK_SUMMARY_PROMPT.format(papers_info="\n\n".join(papers_info), language=language)
    ...
    def _format_paper_list(self, papers: list[Paper], max_count: int = 10) -> str:
        ...
        for idx, paper in enumerate(papers[:max_count], 1):
            authors_str = (
                ", ".join(paper.authors[:2]) + " et al." if len(paper.authors) > 2 else ", ".join(paper.authors)
            )
    ...
    async def get_influential_papers(self, paper_id: str, threshold: float = 0.5) -> list[Paper]:
        """
        获取高影响力参考文献
