...
class FetchPaperDetails(Action):
    ...
    async def run_batch(self, paper_ids: list[str], source: str = "semantic_scholar") -> list[tuple[str, str]]:
        """
        批量获取论文详细信息
        ...
        tasks = [
            self.run(paper_id, source, include_references=False, include_citations=False) for paper_id in paper_ids
        ]
