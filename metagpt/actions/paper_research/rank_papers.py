...
class RankPapers(Action):
    ...
    async def run(
        ...
        try:
            ...
            # 生成报告
            return await self._generate_ranking_report(
                ranked_papers, topic, dim, language
            )
    ...
    async def _rank_by_relevance(
        ...
        # 调用 LLM
        evaluation = await self._aask(prompt)
    ...
    async def _generate_ranking_report(
        ...
        for idx, (paper, score) in enumerate(ranked_papers, 1):
            ...
            authors_str = (
                ...
                if len(paper.authors) > 3 else ", ".join(paper.authors)
            )
        ...
        for paper in papers:
            ...
            if paper.citation_count > 0:
                total_score = (
                    ...
                    0.5 * weights.relevance
                )
            else:
                # 没有引用数据时
                total_score = (
                    abstract_score * weights.citations +
                    recency_score * weights.recency +
                    0.5 * weights.relevance
                )
