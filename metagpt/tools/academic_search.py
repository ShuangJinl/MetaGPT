...
class AcademicSearchTool(BaseModel):
    ...
    async def search_semantic_scholar(
        ...
        params = {"query": query, "offset": offset, "limit": min(max_results, 100), "fields": ",".join(fields)}
        ...
        while retry_count <= self.max_retries:
            ...
            is_rate_limit = (isinstance(data, dict) and data.get("code") == "429") or (
        ...
        return AcademicSearchResult(total=total, papers=papers, offset=offset, next_offset=next_offset)
    ...
    async def search_arxiv(
        ...
        return AcademicSearchResult(total=total, papers=papers, offset=start, next_offset=None)
    ...
    async def get_paper_by_id(self, paper_id: str, source: str = "semantic_scholar") -> Optional[Paper]:
        """
        根据论文 ID 获取详细信息
    ...
    async def get_paper_citations(self, paper_id: str, max_results: int = 20) -> list[Paper]:
        """
        ...
        params = {"fields": ",".join(fields), "limit": min(max_results, 100)}
    ...
    async def get_paper_cited_by(self, paper_id: str, max_results: int = 20) -> list[Paper]:
        """
        获取引用该论文的所有论文列表
    ...
    async def search_multiple(
        ...
        return AcademicSearchResult(
            total=len(unique_papers), papers=unique_papers[:max_results], offset=0, next_offset=None
        )
